...
### 6.2b Resources & Membership (recurring subscription) — *later phase, spec ready*

A separate revenue stream from tokens: a **monthly membership** that unlocks a gated **Resources** library. Built *after* the core (see Phase 6); specced here so the data model and Stripe setup account for it from the start. **Validate on an off-the-shelf platform first** (Podia/Thinkific/Skool) — only build the integrated version below once demand is proven. Content strategy and launch plan live in `MEMBERSHIP_LAUNCH_PLAN.md`.

**Model.**
- A membership is a **Stripe subscription** (recurring), *independent* of tokens — a person can be a token-buyer, a member, or both.
- Launch with **one tier** (e.g. "GCSE Science & Maths", ~£15–19/mo). Structure the schema for **multiple tiers** (by phase/subject) so more can be added without migration.
- **Founding-member** launch price = just a different Stripe price on the same tier.

**Content.**
- A `Resource` = one library item: type (`worksheet | video | past_paper_walkthrough | revision_guide | quiz | parent_guide`), phase/subject/topic tags, a file or video URL, and a `tier` it belongs to.
- Support **drip / scheduled publishing** via a `published_at` date (items appear when due) and a `is_free_sample` flag for public teasers that aid conversion.
- Optional **live sessions**: a `LiveSession` (title, tier, start time, join URL, later a recording URL) — the weekly Q&A that drives retention.

**Access control.**
- Gate `/resources` behind an **active** membership for the matching tier (`admin` always allowed). Check subscription status server-side on every resource fetch, not just in the UI.
- Downgrade gracefully: a lapsed/cancelled member keeps login but sees a "renew to access" state.

**Payments (Stripe subscriptions).**
- Subscribe via **Stripe Checkout** (subscription mode) → **Customer Portal** for the member to upgrade/cancel/update card themselves.
- Webhooks: `customer.subscription.created/updated/deleted` and `invoice.paid/​payment_failed` keep the local `Membership.status` and `current_period_end` in sync. Idempotent on the Stripe subscription/event id.

**Admin.**
- Upload/manage resources (title, type, tags, file/URL, tier, publish date, free-sample flag); schedule live sessions; see member count and churn. Keep it simple — a form + a list.
...
# --- Membership & resources (later phase; see §6.2b) ---
Membership       id, parent_id, tier_key, stripe_subscription_id,
                 status(active|past_due|canceled), current_period_end, created_at
Resource         id, tier_key, type(worksheet|video|past_paper_walkthrough|revision_guide|quiz|parent_guide),
                 title, description, phase, subject, topic, file_url|video_url,
                 published_at, is_free_sample(bool), created_at
LiveSession      id, tier_key, title, starts_at, join_url, recording_url?, created_at
```

**Balance is per (parent, tier)** — computed from `TokenTransaction`, never stored as a mutable field. Purchases carry an `expires_at`; a scheduled job writes `expiry` rows when packs lapse. All webhook writes are **idempotent** keyed on `stripe_session_id` / `cal_booking_uid` / stripe subscription id.
...
8. *(Phase 6 only)* **File storage** for resources (e.g. Supabase Storage / S3) and a **video host** for lesson videos (unlisted YouTube/Vimeo, or Mux). A **membership Stripe subscription price** (+ founding-member price).
...
# Membership (later phase) — subscription price per tier
STRIPE_PRICE_MEMBERSHIP_GCSE=
STRIPE_PRICE_MEMBERSHIP_GCSE_FOUNDING=
...
**Phase 6 — Resources & Membership (only after validating demand).** Stripe subscription + Customer Portal; gated `/resources` library with drip publishing and free samples; optional live-session listings; admin resource upload. See §6.2b. Validate on an off-the-shelf platform first (`MEMBERSHIP_LAUNCH_PLAN.md`).
...
- Recurring subscriptions *for lessons* (the token model is deliberately pay-as-you-go). Note: the **membership** is a subscription, but for *resources*, not lessons — it's Phase 6 (§6.2b), not part of the booking flow.
